...
/// @title ChsbToBorgMigratorV2
/// @notice This contract implements the migration from the CHSB to the BORG token.
/// @dev This is the version 2, which closes the migrator.
/// @author SwissBorg
contract ChsbToBorgMigratorV2 is OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    /// @dev This function is disabled with this upgrade.
    /// @param _amount The amount of $CHSB to migrate.
    function migrate(uint256 _amount) external whenNotPaused {
        revert("MIGRATION_CLOSED");
    }

    /// @notice Burns the migrated CHSB.
    function burnChsb() external whenPaused onlyOwner {
        CHSB.safeTransfer(address(0), CHSB.balanceOf(address(this)));
    }

    /// @notice Withdraws the migrated BORG.
    function withdrawBorg(address _to, uint256 _amount) external whenPaused onlyOwner {
        BORG.safeTransfer(_to, _amount);
    }
